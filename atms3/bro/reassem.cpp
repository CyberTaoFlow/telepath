...
//
// This function is used to copy packet data to internal allocated buffer
//
DataBlock::DataBlock(const u_char* data, int size, int arg_seq,
	...
	if( size+1 < 0 ){
		printf("Wrong size: %d",size);
		block = new u_char[1];
	}else{

		block = new u_char[size+1];
		if ( ! block ){
			reporter->InternalError("Out of memory");
		}

		memcpy((void*) block, (const void*) data, size);
		// we will place zero to the end of the copied block
		// it will help us to print it later
		block[size] = '\0';
...
Reassembler::Reassembler(int init_seq, ReassemblerType arg_type)
	{
	blocks = last_block = 0;
	trim_seq = last_reassem_seq = initial_seq = init_seq;
	}
...
void Reassembler::Reset()
	{
	ClearBlocks();
	trim_seq = last_reassem_seq = initial_seq;
	//total_size = 0;
	}

void Reassembler::NewBlock(/* double q, */ int seq, int len, const u_char* data)
	{
	if ( len == 0 )
		return;
	...
	if ( seq_delta(seq, trim_seq) < 0 )
		...
		// This code will cut the part of the packet because it was previously received !
		// amount_old contains a number of bytes to trim !
...
unsigned int Reassembler::TotalSize() const
	{
	unsigned int size = 0;
