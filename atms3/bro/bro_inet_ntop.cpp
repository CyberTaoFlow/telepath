...
/**
 * bro_inet_ntop() function converts a network format address to presentation format.
 *
    ...
    socklen_t size)
{
	switch (af) {
	case AF_INET:
		return (bro_inet_ntop4((const u_char*)src, dst, size));
	case AF_INET6:
		return (bro_inet_ntop6((const u_char*)src, dst, size));
	default:
		errno = EAFNOSUPPORT;
		return (NULL);
	}
...
/**
 * bro_inet_ntop4() function formats an IPv4 address
 * notes:
 *      (1) uses no statics.
 *      (2) takes a u_char* not an in_addr as input.
 * Author: Paul Vixie, 1996.  Modified by Jon Siwek, 2012, to replace strlcpy
 *
...
/**
 * bro_inet_ntop6() function converts IPv6 binary address into presentation (printable) format.
 * Author: Paul Vixie, 1996.  Modified by Jon Siwek, 2012, for IPv4-translated format
 */
static const char *
