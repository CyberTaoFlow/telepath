...
const int tcp_inactivity_timeout = 60;
...
// This function is called when we are reusing this object
void TCP_Endpoint::Reset()
{
	//contents_processor = NULL;
...
//      contents_file = 0;
	...
	if (is_orig)
	{
		checksum_base = ones_complement_checksum(tcp_analyzer->RespAddr(), 0);
		checksum_base = ones_complement_checksum(tcp_analyzer->OrigAddr(), checksum_base);
	} else {
		checksum_base = ones_complement_checksum(tcp_analyzer->OrigAddr(), 0);
		checksum_base = ones_complement_checksum(tcp_analyzer->RespAddr(), checksum_base);
	}
...
// this function performs tcp checksum check
...
/*
	...
	if (strncmp((const char*)data, "HTTP/1.1 30",10) == 0)
	{
		printf("redirect response\n");
	}
*/
	if ( is_orig == true && len >5 && seq >= contents_processor->LastReassemSeq() )
	{
		if (strncmp((const char*)data, "GET /",5) == 0)
		{
			raw_get++;
			tcp_analyzer->GotRequest("GET");
		} else if (strncmp((const char*)data, "POST /",6) == 0)
		{
			raw_post++;
			tcp_analyzer->GotRequest("POST");
		}
...
int TCP_Endpoint::CheckHistory(uint32 mask, char code)
	...
	return tcp_analyzer->CheckHistory(mask, code);
	}
