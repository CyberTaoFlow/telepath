...
void delete_strings(std::vector<const std::string*>& v)
        {
...
std::string* concatenate(std::vector<const std::string*>& v, int expected_size)
        {
        ...
        if (expected_size == 0)
        {
                expected_size = v[0]->size() * v.size();
        }

        str->reserve(expected_size);

        for ( std::vector<const std::string*>::iterator it = v.begin(); it != v.end(); ++it )
                {
                str->append(**it);
                }
...
std::string* concatenate_and_delete(std::vector<const std::string*>& v, int expected_size)
        {
        if (v.size() == 0)
                return NULL;
        std::vector<const std::string*>::iterator itr = v.begin();
        std::string * str = (std::string *) *itr;
        if (v.size() == 1)
        {
                v.clear();
                return str;
...
void int_to_string(int n, std::string &result)
{
        char digit[12];
        sprintf(digit,"%d",n);
	result.append(digit);
...
void int_to_string(unsigned int n, std::string & result)
{
        char digit[22];
        sprintf(digit,"%u",n);
	result.append(digit);
 ...
 *
 * @param arg_str string to unescape
 * @return A std::string without escaped characters.
 */
std::string get_unescaped_string(const std::string & arg_str)
	...
	*bufpos = 0;
	std::string outstring(buf, bufpos - buf);
...
uint64 calculate_unique_id(size_t pool)
	...
	if ( uid_pool[pool].needs_init )
		...
		if ( ! have_random_seed() )
			...
			uid_instance = HashBytes(&unique, sizeof(unique));
	...
	return HashBytes(&(uid_pool[pool].key), sizeof(uid_pool[pool].key));
	}
