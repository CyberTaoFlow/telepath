...
void FragReassembler::AddFragment(double t, const IP_Hdr* ip, const u_char* pkt)
	...
	if ( ip4 )
		...
		// attack.
		reporter->Weird("fragment_protocol_inconsistency", ip);
		}
	else
		{
	...
	if ( ip->DF() )
		...
		reporter->Weird("fragment_with_DF", ip);
	...
	if ( ! ip->MF() )
		...
		if ( frag_size == 0 )
			...
			{
			reporter->Weird("fragment_size_inconsistency", ip);
		...
		reporter->Weird("excessively_small_fragment", ip);

	if ( upper_seq > MAX_ACCEPTABLE_FRAG_SIZE )
		reporter->Weird("excessively_large_fragment", ip);
...
void FragReassembler::Overlap(const u_char* b1, const u_char* b2, int n)
	...
	if ( memcmp((const void*) b1, (const void*) b2, n) )
		reporter->Weird("fragment_inconsistency", &proto_h);
	else
		reporter->Weird("fragment_overlap", &proto_h);
	}
...
void FragReassembler::BlockInserted(DataBlock* /* start_block */)
	...
	if ( b->next )
		...
		if ( b->upper >= frag_size )
			...
			IP_Hdr proto_h(proto_hdr, false, proto_hdr_len);
			reporter->Weird("fragment_size_inconsistency", &proto_h);
