...
const int tcp_match_undelivered = 1;
const int tcp_max_initial_window = 4096;
const int tcp_max_above_hole_without_any_acks = 4096*10;
const int tcp_excessive_data_without_further_acks = 10 * 1024 *1024;

unsigned long long tot_ack_events = 0;
unsigned long long tot_ack_bytes = 0;
unsigned long long tot_gap_events = 0;
unsigned long long tot_gap_bytes = 0;

const int gap_report = 1;
const int report_gaps_for_partial = 1;
const double gap_report_freq = 10;
...
void TCP_Reassembler::Undelivered(int up_to_seq)
	...
	//if ( DEBUG_tcp_contents )
		{
		DEBUG_MSG("%.6f Undelivered: is_orig=%d up_to_seq=0x%08X, last_reassm=0x%08X, "
...
*/
			...
			dst_analyzer->Undelivered(last_reassem_seq,
								len, is_orig);
...
void TCP_Reassembler::Deliver(int seq, int len, const u_char* data)
	{
		//printf("delivering: len: %d\n%s\n", len, data);
		dst_analyzer->DeliverStream(len, data, is_orig);
...
int TCP_Reassembler::DataSent(double t, int seq, int len,
	...
	if ( last_reassem_seq > seq )
	{
		// we are getting the same packet again
		int skip = min(len, last_reassem_seq - seq );
		seq += skip;
		data += skip;
		len -= skip;
	}
	...
	{
		// do nothing
	} else if ( last_reassem_seq == seq )
	{
		last_reassem_seq += len;
		DeliverBlock(seq, len, data);
	} else {
		NewBlock(seq, len, data);
	}
	//NewBlock(seq, len, data);

	/*
	// this will generate errors in case we have missing acks
