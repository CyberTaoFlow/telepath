...
extern bool net_raw_debug;
extern unsigned long long raw_get;
extern unsigned long long raw_post;

const int tcp_connection_linger = 10;
const int tcp_SYN_timeout = 60;
const int ignore_checksums = 1;
const int tcp_storm_thresh = 1000;
const int tcp_storm_interarrival_thresh = 1;
// in http timout is small
// it is not putty or ftp
// no long connections with long timeouts !
const int tcp_timeout = 60;

// debug number of GET and POST requests
//const bool net_raw_debug = true;
...
#ifndef WIN32
extern RSA * get_ssl_cert(const char * server_ip, unsigned int port);
#endif
...
TCP_Analyzer::TCP_Analyzer(const IPAddr& src_addr, const int src_port, const IPAddr& dst_addr, const int dst_port) /* : TransportLayerAnalyzer(AnalyzerTag::TCP, conn) */
: orig_addr(src_addr), resp_addr(dst_addr), orig_port(src_port), resp_port(dst_port), 
  orig(this, 1), resp(this, 0), orig_reassembler( this, TCP_Reassembler::Forward, true, &orig),
  resp_reassembler(  this, TCP_Reassembler::Forward, false, &resp ), ssl(NULL),
  content_line_orig( &http, true ), content_line_resp(&http, false), http(this, &content_line_orig, &content_line_resp), nrequests(0)
...
*/
	...
	orig.SetPeer(&resp);
	resp.SetPeer(&orig);
	content_line_orig.SetSkipPartial(false);
	content_line_resp.SetSkipPartial(false);

#ifndef WIN32
	DEBUG_MSG("Looking for cert for %s: %d\n", dst_addr.AsURIString().c_str(), dst_port);
	RSA * rsa = get_ssl_cert(dst_addr.AsURIString().c_str(), dst_port);
	if (rsa)
	{
		DEBUG_MSG("RSA certificate found\n");
		ssl = new SSLWrapper_Analyzer(rsa, &content_line_orig, &content_line_resp);
		orig_reassembler.SetDstAnalyzer( ssl );
		resp_reassembler.SetDstAnalyzer( ssl );
	} else {
		ssl = NULL;
		orig_reassembler.SetDstAnalyzer( &content_line_orig );
		resp_reassembler.SetDstAnalyzer( &content_line_resp );	
	}
#else
	orig_reassembler.SetDstAnalyzer( &content_line_orig );
	resp_reassembler.SetDstAnalyzer( &content_line_resp );
#endif
	orig.AddReassembler( &orig_reassembler );
	resp.AddReassembler( &resp_reassembler );
...
//		delete *i;
	...
	//delete orig;
	//delete resp;
	//delete content_line_orig;
	//delete content_line_resp;
	//delete http;

	}

void TCP_Analyzer::Reset(const IPAddr& src_addr, const int src_port, const IPAddr& dst_addr, const int dst_port)
{
	if (ssl)
		delete ssl;
	ssl = NULL;
	...
	orig.Reset();
	resp.Reset();
	orig_reassembler.Reset();
	resp_reassembler.Reset();
	content_line_orig.Reset();
	content_line_resp.Reset();
	http.Reset();
}
...
void TCP_Analyzer::Dump()
{
	DEBUG_MSG("%s:%d => %s:%d\n", orig_addr.AsURIString().c_str(), ntohs(orig_port), resp_addr.AsURIString().c_str(), ntohs(resp_port));
}
...
void TCP_Analyzer::CheckFlagCombos(const TCP_Flags & flags, TCP_Endpoint* endpoint,
...
int TCP_Analyzer::ProcessFlags(double t,
	...
	if ( flags.ACK() )
		ProcessACK(endpoint, peer, ack_seq, is_orig, flags, len);
...
//		Event(connection_first_ACK);
	...
	if ( peer->state == TCP_ENDPOINT_INACTIVE )
		...
		// do not process ack if packet is empty !
		// we have a strange bug when we are getting packet confirmation
		// before actually we see the data !
		else if (len == 0 && (peer->state == TCP_ENDPOINT_ESTABLISHED || peer->state == TCP_ENDPOINT_PARTIAL) )
			return;
		else if ( delta_ack > 0 )
...
//				FlipRoles();
			...
			if ( peer->state == TCP_ENDPOINT_SYN_SENT )
				...
				//    peer->SetState(TCP_ENDPOINT_ESTABLISHED);
				// here.
				Weird("unsolicited_SYN_response (got SYN from server without client SYN)");
				}
...
void TCP_Analyzer::UpdateClosedState(double t, TCP_Endpoint* endpoint,
				int delta_last, const TCP_Flags & flags, int& do_close)
	{
	if ( flags.SYN() )
...
void TCP_Analyzer::UpdateResetState(int len, const TCP_Flags & flags)
	{
	if ( flags.SYN() )
...
//	if (is_orig) {
//		content_line_orig->DeliverStream(len, data, is_orig);
//	} else {
//		content_line_resp->DeliverStream(len, data, is_orig);
//	}
//	http->DeliverStream(len, data, is_orig);
	need_contents = 1;
	return need_contents;
	}

void TCP_Analyzer::CheckRecording(int need_contents, const TCP_Flags & flags)
	{
/*
...
void TCP_Analyzer::DeliverPacket(int len, const u_char* data, const IP_Hdr* ip, int caplen)
	{
//	TransportLayerAnalyzer::DeliverPacket(len, data, orig, seq, ip, caplen);

	TCP_Endpoint* endpoint = &orig;
	bool is_orig = true;
	const struct tcphdr* tp = ExtractTCP_Header(data, len, caplen);

	expiration = network_time + tcp_timeout;
	//DEBUG_MSG("packet expiration: %f\n", expiration);

	if ( ! tp )
		return;

	if (orig_port == tp->th_dport)
	{
		is_orig = false;
		endpoint = &resp;
	} else {
	...
	*/
	...
	if ( ! orig.did_close || ! resp.did_close )
		/* Conn()-> */ SetLastTime(network_time);

	//const IPAddr orig_addr = Conn()->OrigAddr();
	//const IPAddr orig_addr = ip->SrcAddr();

	uint32 tcp_hdr_len = data - (const u_char*) tp;

	int seq_len = len;	// length in terms of sequence space

	DEBUG_MSG("Seq: 0x%08X, Ack: 0x%08X, len %u\n", base_seq - endpoint->StartSeq(), ack_seq - peer->StartSeq(), len);

	if ( ! ProcessFlags(network_time, ip, tp, tcp_hdr_len, len, seq_len,
	...
	if ( endpoint->state == TCP_ENDPOINT_INACTIVE )
		TransitionFromInactive(network_time, endpoint, base_seq, last_seq,
					flags.SYN());

	int delta_last = UpdateLastSeq(endpoint, last_seq, flags);

	endpoint->last_time = network_time;

	int do_close;
	int gen_event;
	UpdateStateMachine(network_time, endpoint, peer, base_seq, ack_seq, last_seq,
...
//		ParseTCPOptions(tp, TCPOptionEvent, this, is_orig, 0);
	...
	if ( len > 0 && (caplen >= len /* || packet_children.size() */ ) &&
	     ! flags.RST() && ! Skipping() )
		need_contents = DeliverData(network_time, data, len, caplen, ip, tp,
						endpoint, base_seq,
						is_orig, flags);
	...
	if (IsFullyClosed())
		{
			expiration = network_time + tcp_connection_linger;
		}
	}
...
*/
	...
	if ( DEBUG_tcp_connection_close )
		{
		DEBUG_MSG("%.6f close_complete=%d tcp_close_delay=%d\n",
				network_time, close_complete, tcp_connection_linger);
		}
...
*/
	http.ConnectionFinished( half_finished );
	is_active = 0;
	}
...
//		static_cast<TCP_ApplicationAnalyzer*>(*i)->ConnectionReset();
	...
	http.ConnectionReset();
	is_active = 0;
	}

bool TCP_Analyzer::HadGap(bool is_orig) const
	{
	const TCP_Endpoint* endp = is_orig ? &orig : &resp;
