...
#define DEBUG_MSG(x,...)	fprintf(stdout, x, ##__VA_ARGS__)
...
#ifndef PRId64
#define PRId64 "lld"  // Format to use with printf.
#define PRIu64 "llu"
#endif
#else
...
std::string* concatenate(std::vector<const std::string*>& v, int expected_size = 0);
void delete_strings(std::vector<const std::string*>& v);
std::string* concatenate_and_delete(std::vector<const std::string*>& v, int expected_size = 0);
