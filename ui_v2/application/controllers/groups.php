...
class Groups extends CI_Controller {
	...
	function get_group() {
		...
		$group_extra = $group->extradata != '' ? json_decode($group->extradata, true) : array();
		$apps   = isset($group_extra['apps']) ? $group_extra['apps'] : array();
		$extra  = isset($group_extra['extra']) ? $group_extra['extra'] : array();
		...
		return_success(array('group' => $group, 'users' => $users_clean, 'perm' => $perm, 'apps' => $apps, 'extra' => $extra));
	...
	function add_group() {
		...
		$extra_data['apps']  = $allowed_apps;
		$extra_data['extra'] = isset($post['items']['extra']) ? $post['items']['extra'] : array();
		...
		if($group_id == 'new') {
			...
			$group_id = $this->ion_auth->create_group($data['name'], $data['description'], array('extradata' => json_encode($extra_data)));
	...
	function set_group() {
		...
		if($group_id != 'new') {
			...
			$group = $this->ion_auth->update_group($group_id, $data['name'], array('description' => $data['description'], 'extradata' => json_encode($extra_data)));
			if(!$group) {
				return_fail($this->ion_auth->errors());
			}
