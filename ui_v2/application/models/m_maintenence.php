...
class M_Maintenence extends CI_Model {
	...
	function patch() {
		...
		// Missing update titles conf
		$this->db->where('action_code','115');
		$row = $this->db->get('config')->row_array();
		if(empty($row)) {
			$this->db->insert('config', array('action_code' => '115', 'name' => 'update_titles', 'value' => '0'));
		}
		
		// Wrongly configured max requests / session
		$this->db->where('action_code','7');
		$this->db->update('config', array('value' => '100000'));
		
		$this->db->where('action_code','62');
		$this->db->update('config', array('value' => '2'));
		
		$this->db->where('action_code','84');
		$this->db->update('config', array('name' => 'smtp_port'));
		
		
		// Missing subdomain_id
		$tables = $this->db->list_tables();
		foreach($tables as $table) {
			// Request scores tables
			if(substr($table, 0, strlen('request_scores_from')) == 'request_scores_from' || $table == 'request_scores') {
				// Query
				$this->log('{"msg":"Correcting data in table '. $table . '"}');
				$sql = "UPDATE $table JOIN pages on $table.page_id = pages.page_id SET $table.subdomain_id = pages.subdomain_id WHERE $table.subdomain_id IS NULL";
				$this->db->query($sql);
			}
		}
		...
		foreach($tables as $table) {
			// Request scores tables
			if(substr($table, 0, strlen('alerts_from')) == 'alerts_from' || $table == 'alerts') {
				// Query
				$this->log('{"msg":"Correcting data in table '. $table . '"}');
				
				if($this->db->query("SHOW INDEX FROM " . $table . " WHERE KEY_NAME = 'SID'")->num_rows() == 1) {
				
					$sql = "ALTER TABLE " . $table . " DROP INDEX SID";
					$this->db->query($sql);
	...
	function work($logfile) {
		
		$this->logfile = $logfile;
		
		// We don't have config table, meaning telepath isn't installed yet, do install
		$tables = $this->db->list_tables();
		...
		$base_path   = '/opt/telepath'; // realpath('.') . '/..';
		...
		$split_tables = array('request_scores', 'attribute_scores', 'alerts');
		
		$tables = $this->db->list_tables();
		
		// Clear split table instances
		foreach($split_tables as $table) {
			if(in_array($table . '_merge', $tables)) {
				$this->db->query('DROP TABLE ' . $table . '_merge');
			}
		}
		...
		foreach($table_files as $file) {
			...
			// Find instances of table split
			if(in_array($table_name, $split_tables)) {
				foreach($tables as $table) {
					if(substr($table, 0, strlen($table_name) + 5) == $table_name . '_from') {
						$queries_alters[$table] = $this->process_table_sql($table_sql, $table);
					}
				}
			}
		...
		foreach($queries_alters as $table_name => $data) {
			$alterTotal += count($data['alter_columns']);		
			$keysTotal += count($data['alter_keys']);
		}
