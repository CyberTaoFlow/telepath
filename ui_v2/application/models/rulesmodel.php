...
class RulesModel extends CI_Model {
	...
	public function get_rule_by_id($rule_id) {
		...
		$fields = array("name", "aspect", "description", "anchor", "att_id", "str_match", 
						...
						"str_similarity", "enable_rule", "not_signal", "radius", "page_id", "ip_neg", "id", "dynamic_type", "att_name");
	...
	public function get_groups($cat) {
		
		$this->db->select("rule_groups.id, name");
		$this->db->select("count(*) num", false);
		$this->db->from("rule_groups, alerts_merge");
		$this->db->where("rule_groups.id = alerts_merge.rule_group");
		$this->db->where("rule_groups.category_id", $cat);
		$this->db->group_by("rule_groups.id");
