telepath.groups = {
	...
	edit: function(record) {
		...
		telepath.util.request('/groups/get_group', { id: record.data.id }, function (data) {
			...
			Ext.getCmp('group_mail_reports').setValue(data['items']['extra']['mail_reports'] == '1' ? 1 : 0);
			Ext.getCmp('group_mail_alerts').setValue(data['items']['extra']['mail_alerts'] == '1' ? 1 : 0);
			...
			telepath.applications.get_list(function (data) {
				...
				telepath.groups.apps_sel = Ext.getCmp('group_applications').getSelectionModel();
				
				if(!telepath.groups.editData['items']['apps'] || telepath.groups.editData['items']['apps'].length == 0) {
					telepath.groups.editData['items']['apps'] = [ -1 ];
				}
				
				var selection = [];
				
				$.each(Ext.getStore('applicationsStore').data.items, function(i, apps_app) { 
					$.each(telepath.groups.editData['items']['apps'], function(r_id, app_id) {
				...
				telepath.groups.apps_sel.select(selection);
				
				telepath.groups.apps_sel.addListener('selectionchange', function(e, o) { 
	...
	save: function() {
		...
		$.each(Ext.getCmp('group_applications').getSelectionModel().selected.items, function (i, group_app) {
			group_applications.push(group_app.data.id);
		});
		
		params['items']['apps']   = group_applications;
		params['items']['users']  = group_users;
		params['items']['perm']   = group_permissions;
		
		params['items']['extra']  = { 
			mail_reports: Ext.getCmp('group_mail_reports').value ? 1 : 0,
			mail_alerts: Ext.getCmp('group_mail_alerts').value ? 1 : 0
		};
	...
	create: function () {
		...
		telepath.groups.editData = { items: { group: { id: 'new' } , perm: [], users: [], apps: [], extra: {  } } };
		telepath.users.loadStore();
		
		Ext.getCmp('group_mail_reports').setValue(1);
		Ext.getCmp('group_mail_alerts').setValue(1);
