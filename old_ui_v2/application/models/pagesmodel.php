...
class PagesModel extends CI_Model {
	...
	private $field_page_app_id 		 = 'pages.app_id';
	private $field_page_subdomain_id = 'pages.subdomain_id';
	...
	public function page_get($page_id, $extended = false) {
	
		$this->db->select($this->field_page_app_id . ',' . $this->field_page_path . ',' . $this->field_page_title . ',' . $this->field_page_subdomain_id);
		
		if($extended) {
			$this->db->select('applications.app_domain as hostname');
			$this->db->select('subdomains.subdomain_name as subdomain_name');
			$this->db->join('subdomains',   $this->tableName . '.subdomain_id = subdomains.subdomain_id', 'left');
			$this->db->join('applications', $this->tableName . '.app_id 		= applications.app_id', 'left');
		}
	...
	function page_search_display_path($search, $exact = false) {
		
		$this->db->select('page_id');
		$this->db->from($this->tableName);
		
		if($exact) {
			$this->db->where('display_path', $search);
		} else {
	...
	function page_search_display_title($search) {
