...
class TableSplit extends CI_Model {
	...
	function get_table_split($table, $min_date = -1, $max_date = -1) {
		...
		$this->info_schema = $this->load->database('information_schema', TRUE);
	...
	function _get_tables($pattern) {
		...
		$this->info_schema->select('table_name');
		$this->info_schema->where('table_type', 'BASE TABLE');
		$this->info_schema->where('table_schema', $this->db->database); // Main db name
		$this->info_schema->where('table_name LIKE', '%' . $pattern . '%');
		
		$this->info_schema->from('TABLES');
		$this->info_schema->order_by('table_name', 'asc');
		
		$query = $this->info_schema->get();
