...
<?php
	...
	foreach($apps as $app) {
		...
		echo '	upstream application_' . $app['app_id'] . '_upstream {' . "\n";
...
<?php
	...
	foreach($apps as $app) {
		...
		echo '		server_name ' . $app['app_domain'] . ';'  . "\n";
		
		if(intval($app['ssl_flag']) == 1) {
		
			echo '		listen ' . $app['ssl_server_port'] . ' ssl;' . "\n";
			echo '		ssl_certificate ' . $certs_dir . 'application_' . $app['app_id'] . '_certificate.crt'  . ";\n";
			echo '		ssl_certificate_key ' . $certs_dir . 'application_' . $app['app_id'] . '_private_key.key'  . ";\n";
...
<?php
	echo '			proxy_pass		http' . (intval($app['ssl_flag']) == 1 ? 's' : '') . '://application_' . $app['app_id'] . '_upstream;';
?>
